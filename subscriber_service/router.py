...
async def get_user_subs(
    ...
    query = await session.execute(
        select(SubDB)
        .options(joinedload(SubDB.frequency))
...
async def create_new_sub(
    ...
    new_sub = SubDB(
        **sub.model_dump(), payment_date=payment_date, user_id=current_user["user_id"]
    )
...
async def update_sub(
    ...
    query = await session.execute(
        select(SubDB).where(
            SubDB.id == sub_id, SubDB.user_id == current_user["user_id"]
        )
    )
    ...
    await remove_reminder(current_user["email"], result.title, result.frequency_id)
